...
/**
 * Récupérer les partenaires à proximité
 */
export async function getNearbyPartners(latitude, longitude, radius = 5000) {
  try {
    const response = await apiService.get('/admin/partners/nearby', {
      params: { latitude, longitude, radius }
    });
    
    return {
      items: (response.data.partners || response.data.items || []).map(mapBackendPartner),
 ...
 */
export async function getPartnerStats(partnerId, period = '30d') {
  try {
...
export async function getPartnerAnalytics(params = {}) {
  try {
    const response = await apiService.getPartnerAnalytics(params);
