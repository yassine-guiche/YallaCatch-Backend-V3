...
export default function RewardsManagement() {
  ...
  useRewardsUpdates({
    onRewardUpdate: () => {
      loadRewards();
      toast.info('Recompense mise a jour');
    },
    onRedemptionCreated: () => {
      loadExchangeHistory();
    },
  ...
  const loadFeaturedRewards = async () => {
    try {
      ...
      console.error('Erreur chargement recompenses vedettes:', err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingReward) {
        ...
        toast.success('Recompense ajoutee');
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success('Recompense supprimee');
      await loadRewards();
      await loadFeaturedRewards();
    } catch (err) {
      toast.error('Erreur: ' + err.message);
  ...
  const getCategoryBadge = (category) => {
    const categoryConfig = {
      ...
      digital: { color: 'bg-green-100 text-green-800', label: 'Numerique' },
      experience: { color: 'bg-orange-100 text-orange-800', label: 'Experience' }
    };
  ...
  const sponsoredBadge = (reward) => {
    ...
    return (
      <Badge className="bg-amber-100 text-amber-800 flex items-center gap-1">
        ...
        Sponsorise {reward.metadata?.sponsorName ? `- ${reward.metadata.sponsorName}` : ''}
      </Badge>
    );
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <ConfirmDialog
        ...
        title="Supprimer la recompense"
        description="Etes-vous sur de vouloir supprimer cette recompense ? Cette action est irreversible."
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">Gestion des Recompenses</h1>
          <p className="text-gray-500 mt-1">
            {rewards.length} recompense{rewards.length > 1 ? 's' : ''} disponible{rewards.length > 1 ? 's' : ''}
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium">Total Recompenses</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between items-center text-xs text-gray-500">
                        ...
                        <span>Validite: {reward.validityPeriod || 30}j</span>
                      </div>
        ...
        <TabsContent value="categories" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Categories du Marketplace</CardTitle>
              <CardDescription>Organisation des recompenses par categorie</CardDescription>
            </CardHeader>
            <CardContent>
              {categories.length === 0 ? (
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-gray-500">
                              {category.count || 0} recompense{(category.count || 0) > 1 ? 's' : ''}
                            </p>
                          </div>
        ...
        <TabsContent value="featured" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Recompenses Vedettes</CardTitle>
              <CardDescription>Recompenses mises en avant dans le marketplace</CardDescription>
            </CardHeader>
            <CardContent>
              {featuredRewards.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  Aucune recompense vedette
                </div>
              ) : (
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle>Historique des echanges</CardTitle>
              <CardDescription>Derniers echanges de recompenses</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                {editingReward ? 'Modifier les informations de la recompense' : 'Ajouter une nouvelle recompense au marketplace'}
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="digital">Produit numerique</option>
                ...
                <div className="col-span-2 space-y-2 border-t pt-3">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded"
                    />
                    <span className="text-sm font-medium">Recompense sponsorisee (requiert un partenaire)</span>
                  </label>
                ...
                <div className="col-span-2">
                  <label className="flex items-center gap-2">
                    <input
                      ...
                      className="rounded"
                    />
                    <span className="text-sm font-medium">Recompense active</span>
                  </label>
                </div>
              </div>
