...
import { IPrizeDocument, IPrize } from '@/types';
...
export class ProximityService {
    ...
    static async getProximityPrizes(
        ...
        try {
            ...
            typedLogger.error('Get proximity prizes error', { error: error instanceof Error ? error.message : String(error), userId, location });
            throw error;
        }
    }
    ...
    static async getDynamicHints(
        ...
        try {
            ...
            typedLogger.error('Get dynamic hints error', { error: error instanceof Error ? error.message : String(error), userId, prizeId });
            return [];
        }
    }
    ...
    static async validateCatchAttempt(
        ...
        try {
            ...
            typedLogger.error('Validate catch attempt error', { error: error instanceof Error ? error.message : String(error), userId, prizeId });
    ...
    static async updateLocationAndGetProximity(
        ...
        try {
            ...
            typedLogger.error('Update location and get proximity error', { error: error instanceof Error ? error.message : String(error), userId, location });
            throw error;
        }
    }
    ...
    static async getUnityOptimizedProximity(
        ...
        try {
            ...
            typedLogger.error('Get Unity optimized proximity error', { error: error instanceof Error ? error.message : String(error), userId, location });
            throw error;
        }
    }
    ...
    private static async getNearbyPrizes(
        ...
        try {
            ...
            const prizes = await Prize.find({
                ...
                .select('title category rarity points location distribution content metadata visibility expiresAt status quantity claimedCount')
                .lean();

            return prizes as unknown as IPrizeDocument[];
        } catch (error) {
            typedLogger.error('Get nearby prizes error', { error: error instanceof Error ? error.message : String(error), location, radiusMeters });
            return [];
        }
    }

    private static async filterClaimedPrizes(userId: string, prizes: IPrizeDocument[]): Promise<IPrizeDocument[]> {
        try {
            ...
            typedLogger.error('Filter claimed prizes error', { error: error instanceof Error ? error.message : String(error), userId });
            return prizes;
        }
    }
    ...
    private static categorizeByZones(
        ...
        for (const prize of prizes) {
            ...
            const proximityPrize: ProximityPrize = {
                ...
                animation: (prize as any).content?.animation || 'standard',
    ...
    private static calculateEstimatedValue(prize: IPrizeDocument): number {
        ...
        // Bonus multiplier - using optional chaining with explicit typing if needed
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if ((prize as any).content?.bonusMultiplier) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            value *= (prize as any).content.bonusMultiplier;
        }
    ...
    private static async performAntiCheatValidation(
        ...
        try {
            ...
            typedLogger.error('Anti-cheat validation error', { error: error instanceof Error ? error.message : String(error), userId });
    ...
    private static async storeLocationUpdate(
        ...
        try {
            ...
            typedLogger.error('Store location update error', { error: error instanceof Error ? error.message : String(error), userId });
        }
    }
    ...
    private static async cacheProximityResult(
        ...
        try {
            ...
            typedLogger.error('Cache proximity result error', { error: error instanceof Error ? error.message : String(error), userId });
        }
    }

    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    private static async detectZoneTransitions(userId: string, currentResult: ProximityResult): Promise<Array<{ type: string; prizeId: string; from: string; to: string }>> {
        try {
            ...
            const transitions: Array<{ type: string; prizeId: string; from: string; to: string }> = [];
            ...
            typedLogger.error('Detect zone transitions error', { error: error instanceof Error ? error.message : String(error), userId });
            return [];
        }
    }

    private static async triggerZoneTransitionEvents(userId: string, transitions: Array<{ type: string; prizeId: string; from: string; to: string }>): Promise<void> {
        try {
            ...
            typedLogger.error('Trigger zone transition events error', { error: error instanceof Error ? error.message : String(error), userId });
        }
    }
}
