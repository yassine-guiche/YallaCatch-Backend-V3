...
/**
 * Obtenir l'historique des points d'un utilisateur (admin)
 */
export async function getUserPointsHistory(userId, params = {}) {
  try {
    const response = await apiService.get(`/admin/users/${userId}/points/history`, params);
    ...
    throwNormalized(error, "Impossible de charger l'historique des points");
  }
}
...
export async function getUserStats() {
  try {
    // Call the admin analytics endpoint for users
    const response = await apiService.get('/admin/analytics/users', { period: 'month' });
    const data = response.data || response;
    return {
      total: data.totalUsers || 0,
      active: data.activeUsers || 0,
      banned: data.bannedUsers || 0,
    ...
    return { total: 0, active: 0, banned: 0, newUsers: 0, retentionRate: 0, topUsers: [], userGrowth: [] };
  }
}
