 ...
 * @param {Object} data - { count, prefix, pointsValue, rewardId, expiresAt }
 */
export async function generateCodes(data) {
  try {
    const payload = {
      ...
      pointsValue: data.rewardId ? 0 : (data.pointsValue || 100),
...
/**
 * Activate a deactivated promo code
 * @param {string} codeId 
 */
export async function activateCode(codeId) {
  try {
    const response = await apiService.patch(`/admin/codes/${codeId}/activate`);
    return {
      success: response?.success ?? true,
    ...
    throwNormalized(error, 'Impossible d\'activer le code');
  }
}
