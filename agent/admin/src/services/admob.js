...
const admobService = {
  ...
  getAnalytics: async (params = {}) => {
    ...
    const response = await api.get(`/admin/admob/analytics?${queryParams.toString()}`);
    return response.data;
  },

  // Get current AdMob configuration (Admin)
  getConfig: async () => {
    const response = await api.get('/admin/admob/config');
    return response.data;
  },

  // Update AdMob configuration (Admin)
  updateConfig: async (config) => {
    const response = await api.patch('/admin/admob/config', config);
    return response.data;
  },
