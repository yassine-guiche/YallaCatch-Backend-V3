...
export async function getUnityPerformanceReport(params = {}) {
  try {
    const response = await apiService.get(`${GAME_CONTROL_BASE}/metrics/unity-performance`, { params });
    return response.data || response;
  } catch (error) {
...
export async function getUnityRealtimeMetrics() {
  try {
    const response = await apiService.get(`${GAME_CONTROL_BASE}/metrics/realtime`);
    return response.data || response;
  } catch (error) {
 ...
 * @param {string} metric - Metric name (e.g. 'business.users.active_count')
 * @param {string} range - Time range ('1h', '24h', '7d', '30d')
 */
export async function getMetricsHistory(metric, range = '1h') {
  try {
    const response = await apiService.get(`${GAME_CONTROL_BASE}/metrics/history`, { metric, range });
    return response.data || response;
  } catch (error) {
