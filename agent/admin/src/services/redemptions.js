...
export async function getRedemption(redemptionId) {
  try {
    const response = await api.get(`/admin/redemptions/${redemptionId}`);
...
export async function updateRedemptionStatus(redemptionId, status, actor = {}) {
  try {
    const response = await api.patch(`/admin/redemptions/${redemptionId}/status`, {
      status,
    });
...
export async function updateRedemptionTracking(redemptionId, tracking, actor = {}) {
  try {
    const response = await api.patch(`/admin/redemptions/${redemptionId}/tracking`, {
      tracking,
    });
...
export async function cancelRedemption(redemptionId, reason, actor = {}) {
  try {
    const response = await api.post(`/admin/redemptions/${redemptionId}/cancel`, {
      reason,
    });
...
export async function getRedemptionStatistics() {
  try {
    const response = await api.get('/admin/redemptions/statistics');
...
export async function exportRedemptions(filters = {}) {
  try {
    const response = await api.get('/admin/redemptions/export', {
      params: filters,
      responseType: 'blob',
    });
...
export async function bulkUpdateRedemptionStatus(redemptionIds, status, actor = {}) {
  try {
    const response = await api.post('/admin/redemptions/bulk-update', {
      redemptionIds,
      status,
    });
