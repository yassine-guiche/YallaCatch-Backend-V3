...
function StatCard({ title, value, subtitle, IconComponent, trend, trendValue, color = 'blue', loading }) {
  const colorClasses = {
      ...
      setActivity(prev => [
        {
          ...
          description: data.reward?.name || 'Récompense échangée',
          createdAt: new Date(),
          success: true,
          category: 'redemption'
        },
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          <p className="text-gray-500 mt-1 flex items-center gap-2">
            ...
            {lastUpdate && (
              <span className="text-xs text-gray-400">
                - Mis à jour {formatRelativeDate(lastUpdate)}
              </span>
            )}
          </p>
        </div>
      ...
      <div className="grid gap-4 grid-cols-2 lg:grid-cols-4">
        <StatCard
          title="Revenus AdMob"
          value={`${monetizationStats.adRevenue?.toFixed(2) || '0.00'} €`}
          subtitle={`${monetizationStats.adRevenueToday?.toFixed(2) || '0.00'} € aujourd'hui`}
        ...
        <StatCard
          ...
          subtitle={`${pointsStats.spent?.toLocaleString() || 0} dépensés`}
          IconComponent={Coins}
          color="orange"
          loading={loading && !stats}
        />
            ...
            <div className="flex items-center justify-between">
              <div>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Activité sur {selectedPeriod === '1d' ? 'les dernières 24h' : `les ${selectedPeriod.replace('d', '')} derniers jours`}
                </CardTitle>
                <CardDescription>Évolution des captures et rédemptions</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Répartition des activités principales</CardDescription>
          </CardHeader>
          <CardContent>
