...
import { AlertCircle, CheckCircle, XCircle, Eye, MessageSquare, RefreshCw, Search, Copy } from 'lucide-react';
...
export default function ReportsManagement() {
  ...
  const getStatusBadge = (status) => {
    const variants = {
      ...
      investigating: { variant: 'secondary', icon: Search, label: 'En investigation' },
      resolved: { variant: 'success', icon: CheckCircle, label: 'Résolu' },
      rejected: { variant: 'destructive', icon: XCircle, label: 'Rejeté' },
      duplicate: { variant: 'outline', icon: Copy, label: 'Doublon' }
    };
              ...
              <TableBody>
                {reports.map((report) => (
                  <TableRow key={report._id}>
                    ...
                    <TableCell>
                      {report.userId?.displayName || report.userId?.username || report.reporterName || 'N/A'}
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={!['pending', 'investigating'].includes(report.status)}
                        >
